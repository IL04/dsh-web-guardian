...
function loginPage(error = false) {
  const notice = error ? '<p class="error" role="alert">账户或密码不正确，请重试。</p>' : ''
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>登录 · DSH Web</title><style>:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 15% 15%,#203d78 0,transparent 32rem),radial-gradient(circle at 85% 80%,#184c46 0,transparent 28rem),#0a1020;color:#edf2ff;font:16px/1.5 system-ui,-apple-system,"Segoe UI",sans-serif}.card{width:min(470px,calc(100% - 32px));padding:38px;border:1px solid #ffffff1f;border-radius:24px;background:#101a31d9;box-shadow:0 24px 80px #0007;backdrop-filter:blur(18px)}.brand{display:grid;grid-template-columns:72px minmax(0,1fr);gap:18px;align-items:center;margin-bottom:30px}.logo{grid-row:1 / span 2;width:72px;height:72px;display:grid;place-items:center;border:1px solid #ffffff2e;border-radius:19px;background:linear-gradient(135deg,#34558e,#6355a4);box-shadow:inset 0 1px #ffffff26}.logo img{width:50px;height:50px}.brand-copy{min-width:0}h1{font-size:27px;line-height:1.2;letter-spacing:-.04em;margin:0 0 7px}.sub{margin:0;color:#aebbd5;font-size:15px}.error{margin:-10px 0 18px;color:#ffb4b4;font-size:14px}label{display:block;margin:15px 0 7px;color:#cbd5ed;font-size:14px;font-weight:600}input{width:100%;border:1px solid #ffffff26;border-radius:11px;padding:12px 13px;background:#071024;color:inherit;font:inherit;outline:none}input:focus{border-color:#86b2ff;box-shadow:0 0 0 3px #5a92ff2c}button{width:100%;margin-top:24px;border:0;border-radius:11px;padding:12px 16px;background:linear-gradient(135deg,#78aaff,#826cf7);color:#071024;font:700 16px inherit;cursor:pointer}.foot{margin:22px 0 0;color:#8694b1;font-size:12px;text-align:center}@media(max-width:420px){.card{padding:28px 24px}.brand{grid-template-columns:60px minmax(0,1fr);gap:14px}.logo{width:60px;height:60px;border-radius:16px}.logo img{width:42px;height:42px}h1{font-size:23px}.sub{font-size:14px}}</style></head><body><main class="card"><header class="brand"><div class="logo"><img src="/favicon.svg" alt="DSH" width="50" height="50"></div><div class="brand-copy"><h1>欢迎使用 DSH Web</h1><p class="sub">DeepSeek Harness 局域网控制台</p></div></header>${notice}<form method="post" action="/login"><label for="username">账户</label><input id="username" name="username" required autofocus autocomplete="username"><label for="password">密码</label><input id="password" name="password" type="password" required autocomplete="current-password"><button type="submit">登录</button></form><p class="foot">受密码保护的本机服务</p></main></body></html>`
}
...
const uuidShim = `<script data-dsh-web-uuid-shim>(function(){const native=globalThis.crypto;if(native&&typeof native.randomUUID==='function')return;const uuid=()=>{const b=new Uint8Array(16);if(native&&typeof native.getRandomValues==='function')native.getRandomValues(b);else for(let i=0;i<b.length;i++)b[i]=Math.floor(Math.random()*256);b[6]=b[6]&15|64;b[8]=b[8]&63|128;const h=Array.from(b,x=>x.toString(16).padStart(2,'0')).join('');return[h.slice(0,8),h.slice(8,12),h.slice(12,16),h.slice(16,20),h.slice(20)].join('-')};try{if(native)Object.defineProperty(native,'randomUUID',{value:uuid,configurable:true})}catch(_){ }if(globalThis.crypto&&typeof globalThis.crypto.randomUUID==='function')return;try{const proxy=new Proxy(native||{},{get(t,k){if(k==='randomUUID')return uuid;const v=Reflect.get(t,k,t);return typeof v==='function'?v.bind(t):v}});Object.defineProperty(globalThis,'crypto',{value:proxy,configurable:true})}catch(_){try{globalThis.crypto={randomUUID:uuid}}catch(__){ }}})()</script>`
...
async function start() {
  ...
  const server = http.createServer((request, response) => {
    ...
    if (pathname === '/favicon.svg') return proxy(request, response)
